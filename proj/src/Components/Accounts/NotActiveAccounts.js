...
export const NotActiveAccounts = () => {

    const [data, setData] = useState([]);  

function UserStatus(SSN){
  Axios.post('http://localhost:3003/activateAccount', {
    UserSSN: SSN
}).then(() => {
    console.log("success")
...
{/*
 function refreshPage() { 
  ...
  useEffect(() => {  
    Axios  
        .get("http://localhost:3003/accountsNotActive")  
...
return (  
        ...
        <table className="table" >  
                ...
                <tbody>  
                  ...
                  <td>{item.Description}</td> 
                            <td><button className="btn btn-primary" onClick={(() => UserStatus(item.SSN)
                            )}>تفعيل</button></td> 
                        </tr>  
                    })}  
                </tbody>  
    ...
    export default NotActiveAccounts;
